...
class Strings():
    WARNING = FG_YELLOW + 'Warning' + ALL_OFF + ': '
    ERROR = FG_RED + 'Error' + ALL_OFF + ': '

    RUNNING_TESTCASE_START = FG_CYAN + BOLD + '\n========== Running test '
    ...
    RUNNING = FG_MAGENTA + 'Running' + ALL_OFF + ': '
    STATUS = '\nStatus: '
    CONSOLE_DIFFERENCES = FG_BLUE + '\nConsole output differences' + ALL_OFF + ': '
    OUTPUT_DIFFERENCES = FG_BLUE + '\nTransaction summary file differences' + ALL_OFF + ': '
    TRAILING_NEWLINE = '<trailing newline>' + ALL_OFF
...
class TestResult(Enum):
    NO_RESULT = FG_YELLOW + 'No result'
    PASSED = FG_GREEN + 'Passed'
    FAILED_OUTPUT = FG_RED + 'Failed' + ALL_OFF + ' (transaction summary file differs)'
    FAILED_CONSOLE_OUTPUT = FG_RED + 'Failed' + ALL_OFF + ' (console output differs)'
    FAILED_BOTH = FG_RED + 'Failed' + ALL_OFF + ' (both transaction summary file & console output differ)'

# The commad to run for executing the script to test
FRONTEND_COMMAND = ['cmd', '/c', 'python'] if os.name == 'nt' else ['python3']
...
def print_status_message(message):
    print(Strings.STATUS + BOLD + message + ALL_OFF)

# Print "Error" in red followed by a message
def print_error_message(message):
    print(Strings.ERROR + message + ALL_OFF)

# Print "Warning" in yellow followed by a message
def print_warning_message(message):
    print(Strings.WARNING + message + ALL_OFF)
...
def print_test_statistics(total, skipped, passed, failed):
    ...
    print(Strings.PASSED_TESTS + ('0' if passed == 0 else FG_GREEN + str(passed) + ' = ' + str(passed / total * 100) + '%') + ALL_OFF)
    print(Strings.FAILED_TESTS + ('0' if failed == 0 else FG_RED + str(failed) + ' = ' + str(failed / total * 100) + '%') + ALL_OFF)
    ...
    for differingLine in differingLines:
        ...
        for prefix in ('---', '+++'):
            ...
            if(differingLine.startswith('-')):
                print(FG_RED + differingLine)
            elif(differingLine.startswith('+')):
                print(FG_GREEN + differingLine)
...
def main():
    ...
    for currentDirectoryGerenrator in os.walk("."):
        ...
        if(FileNames.TESTCASE_FOLDER_PREFIX in currentDirectory):
            ...
            frontendProcess = run(currentCommandToRun, stdout=PIPE, stderr=DEVNULL, input=currentInputLines, universal_newlines=True)

            # Get the console output
            actualConsoleOutput = ansi_escape.sub('', str(frontendProcess.stdout))
