...
class user(object):
    ...
    def checkAccNum(self, accNum):
        if (accNum // 1000000 > 0) and (accNum // 1000000 < 10):
            return True
    ...
    def transfer(self):
        ...
        # if transfer valid then process transfer
        if self.checkAccNum(fromAcc) and self.checkAccNum(toAcc) and self.checkAmount(amount):
            ...
            content = "XFR " + str(fromAcc) + ' ' + '$' + str(
                float("{:.2f}".format(int(amount) / 100))) + ' '+ str(toAcc) + ' '+ str(self.acctName) + '\n'
            self.writeSummary("transactionSummaryFile.txt", content)
            print(self.balance)
            print('successfully')
            return self.chooseFunction()
